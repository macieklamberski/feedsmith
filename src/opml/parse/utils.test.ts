...
import { parseNumber, parseString } from '../../common/utils.js'
import { parseBody, parseCsvOf, parseHead, parseOpml, parseOutline } from './utils.js'
...
describe('parseCsvOf', () => {
  ...
  it('should handle strings with consecutive commas', () => {
    const value = '1,,3'
    const expected = [1, 3]
  ...
  it('should return undefined for non-string inputs', () => {
    expect(parseCsvOf(null, parseNumber)).toBeUndefined()
    expect(parseCsvOf(undefined, parseNumber)).toBeUndefined()
    expect(parseCsvOf({}, parseNumber)).toBeUndefined()
    expect(parseCsvOf([], parseNumber)).toBeUndefined()
  })
})
