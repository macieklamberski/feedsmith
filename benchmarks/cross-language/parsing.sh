...
run_benchmark() {
  ...
  local json
  json=$(mktemp)
  hyperfine --warmup 3 --min-runs 10 --export-json "$json" \
    ...
    --command-name 'feedparser (python)' "$SCRIPT_DIR/.venv/bin/python3 $SCRIPT_DIR/parsing-feedparser.py $FILES_DIR/$feed_dir $feed_format" \
